...
# meta developer: большая часть кода @kepperok добавления и улучшения @tot_882 (+ @Murex55)
@loader.tds
class EmeraldMiner(loader.Module):
    strings = {
        "name": "EmeraldMiner",
    ...
    async def watcher(self, message):
        ...
        if hasattr(message, 'from_id') and hasattr(message, 'chat_id') and message.from_id == 7084173311 and message.chat_id == 7084173311 and "Руда на уровень" in message.raw_text:
            a["clicks"] += 1
            self.set("kol_cases", a)
            
        if hasattr(message, 'from_id') and hasattr(message, 'chat_id') and message.from_id == 7084173311 and message.chat_id == 7084173311 and "Найден" in message.raw_text:
        ...
        if hasattr(message, 'from_id') and message.from_id == 7084173311 and message.chat_id == 7084173311 and "Плазма +" in message.raw_text:
            plpt = r"\d+"
    ...
    async def continue_mining(self):
        while self.get('mm'):
            await self.client.send_message("@mine_evo_emerald_bot", 'коп')
