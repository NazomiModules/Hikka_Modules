...
class NazomiAutoClanRentalPRD(loader.Module):
    ...
    async def custom_rent_handler(self, event):
        ...
        async with self.lock:
            try:
                nick_message = await self.client.send_message(chat_id, 'Ник', reply_to=event.message.id)
                response = await self.wait_for_event(
                    events.NewMessage(func=lambda e: (e.chat_id == chat_id and e.sender_id == 5522271758 and e.raw_text.startswith('🏷') and 'Ник' in e.raw_text)), timeout=3)

                if response is None:
                    await event.reply('<b><emoji document_id=5382194935057372936>⌛</emoji> Бот не ответил на команду ника</b>')
                    return

                match = re.search(r'\s*Ник игрока\s*:\s*(\S+)', response.raw_text)
                if match:
