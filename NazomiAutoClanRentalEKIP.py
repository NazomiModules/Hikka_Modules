...
Имя: NazomiAutoClanRentalEKIP
Описание: Модуль для авто-выдачи в аренду экипировки в MineEVO специально для кланов
--------------------------------------------------------------------
'''
...
@loader.tds
class NazomiAutoClanRentalEKIP(loader.Module):
    """Модуль для авто-выдачи в аренду экипировки в MineEVO специально для кланов"""
    strings = {
        "name": "NazomiAutoClanRentalEKIP"
    }
    ...
    config = loader.ModuleConfig(
        ...
        loader.ConfigValue(
            "time",
            "5м",
            "Время аренды экип (например, 5м, 1ч, 1д)"
        ),
        loader.ConfigValue(
            "command_trigger",
            "~Дай",
            "Префикс для команды выдачи экип"
        )
    )
    ...
    def __init__(self):
        ...
        self.command_trigger = "~Выдайэкип"
    ...
    async def custom_rent_handler(self, event):
        ...
        if len(args) != 1:
            await event.reply(
                "<emoji document_id=5382194935057372936>📄</emoji><b> Неверный формат команды. Используйте: {} <экипировка></b>".format(self.command_trigger)
            )
            return
        subject = args[0]
    ...
    @loader.command()
    async def nesc(self, message: Message):
        """Установить чат для выдачи экипировки, нужно просто написать в нужном чате"""
        try:
    ...
    @loader.command()
    async def nebla(self, message: Message):
    ...
    @loader.command()
    async def nebld(self, message: Message):
    ...
    @loader.command()
    async def nebls(self, message: Message):
    ...
    @loader.command()
    async def nest(self, message: Message):
        """Установить время выдачи экипировки"""
    ...
    @loader.command()
    async def nesp(self, message: Message):
        """Изменить префикс выдачи экипировки"""
        args = utils.get_args(message)
        if not args:
            await message.edit(
        ...
        await message.edit(f"<b><emoji document_id=5870525453822859417>🏷</emoji> Префикс выдачи изменён на </b><code>{new_command}</code>")
    ...
    async def _process_interaction(self, message: Message, subject: str, nickname: str, time_str: str, silent: bool = False) -> None:
        equipment_emoji = self.equipment_dict.get(subject.lower())
        ...
        try:
            ...
            if not subject_found:
                raise ValueError(f"<emoji document_id=5260293700088511294>⛔️</emoji><b> Экипировка {equipment_emoji} не найдена!</b>")
